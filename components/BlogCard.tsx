...
export default function BlogCard(props: BlogCardProps) {
    return (
        <div className="flex flex-col bg-white border-black rounded-xl w-full shadow-xl">
            <div className="h-[250px] w-full relative ">
                <Image style={{filter: 'brightness(1)'}} src={props.image_url} objectFit={"cover"} layout={"fill"}
                       alt="Чаи" className="rounded-t-xl"/>
            </div>
            <div className="p-4 relative">
                <h4 className="font-black">{props.title}</h4>
                <p className="text-[14px]">{props.description}</p>
                {/* <div className="w-full h-full absolute bg-black top-0 left-0 opacity-10 rounded-b-xl"></div> */}
                <Link href={props.article_url}>
                    <button
