...
export async function GET(req: NextRequest) {
    try {
        ...
        return NextResponse.json({ success: true, syrups })
    } catch (error) {
...
export async function POST(req: NextRequest) {
    try {
        ...
        return NextResponse.json({ success: true, syrup }, { status: 201 })
    } catch (e: any) {
        if (e?.code === 'P2002') {
            return NextResponse.json({ error: 'Syrup with this name already exists' }, { status: 400 })
        }
        console.error('Error creating syrup:', e)
